...
router.post('/email', async (req, res) => {
  ...
  const safeFilename = filename || `MrTom_Reporte_${today.replace(/\//g, '-')}.pdf`;

  try {
    const { error } = await resend.emails.send({
      from:    'MrTom Reporte <onboarding@resend.dev>',
      to:      [to],
      subject: `Reporte Ejecutivo MrTom — ${today}`,
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;color:#1f2937">
          <div style="background:#1e3a5f;padding:24px 32px;border-radius:12px 12px 0 0">
            <h1 style="color:#fff;margin:0;font-size:20px">MrTom</h1>
          ...
          <div style="background:#f9fafb;padding:24px 32px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb">
            ...
            <p style="margin:0;font-size:12px;color:#9ca3af">Enviado desde MrTom — Control Presupuesto Fauna BTL</p>
          </div>
        </div>
      `,
      attachments: [{
