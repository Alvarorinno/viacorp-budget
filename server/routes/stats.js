...
router.get('/', async (req, res) => {
  try {
    const rawEvents = await sql`SELECT *, (presupuesto - costo) as mb FROM events`;
    const events = rawEvents.map(r => ({ ...r, presupuesto: Number(r.presupuesto || 0), costo: Number(r.costo || 0), mb: Number(r.presupuesto || 0) - Number(r.costo || 0) }));

    const totalPresupuesto = events.reduce((s, e) => s + e.presupuesto, 0);
    const totalCosto = events.reduce((s, e) => s + e.costo, 0);
    const totalMB = totalPresupuesto - totalCosto;
    const mbPct = totalPresupuesto > 0 ? (totalMB / totalPresupuesto) * 100 : 0;
    const totalFacturado = events.filter(e => e.factura).reduce((s, e) => s + e.presupuesto, 0);
    ...
    for (const e of events) {
      ...
      byMonth[m].presupuesto += e.presupuesto;
      byMonth[m].costo += e.costo;
      byMonth[m].mb += e.mb;
      if (e.factura) byMonth[m].facturado += e.presupuesto;
      byMonth[m].count += 1;
    }
    const monthlyData = MONTH_ORDER.filter(m => byMonth[m]).map(m => ({
      ...byMonth[m],
      mbPct: byMonth[m].presupuesto > 0 ? (byMonth[m].mb / byMonth[m].presupuesto) * 100 : 0
    }));
    ...
    for (const e of events) {
      ...
      byClient[c].presupuesto += e.presupuesto;
      byClient[c].mb += e.mb;
      byClient[c].count += 1;
    }
